...
def enhance(
    audio: tuple[int, np.ndarray], snr_threshold: int
...
def generate(inputs: str, speaker: str) -> Generator:
    if speaker == "Imposter":
        ...
        for token_str in streamer:
            ...
            simplified_text = "".join(token_strs)
            traditional_text = simplify_to_traditional(simplified_text)
            yield traditional_text 
...
def transcribe(audio: tuple[int, np.ndarray], speaker: str) -> str:
    ...
    result = pipelines["automatic-speech-recognition"](
